...
mongoClient.connect((err) => {
  ...
  app.get('/icons', async (req, res) => {
    ...
    try {
      const userProfile = await playerCollection.findOne({ 'session.token': session_token });

      if (!userProfile) {
        console.log('No user profile found for session token, redirecting to login page');
        return res.redirect('/login');
      }
      ...
      if (needsDBUpdateForDefaults) {
        console.log(`Applying default fields for user ${userId}:`, defaultsToSet);
        try {
          await playerCollection.updateOne({ '_id': userId }, { $set: defaultsToSet });

        } catch (err) {
          console.error(`Database error setting default fields for user ${userId}:`, err);
        }
      }
      ...
      if (newlyUnlockedIcons.length > 0) {
        ...
        try {
          ...
          console.error(`Database error updating unlocked icons for user ${userId}:`, err);
        }
      }
      ...
      for (const iconName in ALL_ICONS_METADATA) {
        ...
        finalAllIconsData[iconName] = {
          ...
          unlock: (!isOwned && meta.unlock !== undefined) ? meta.unlock?.text : undefined,
        };
      }
    ...
    function loadIcons() {
      const iconsPath = path.join(__dirname, '..', 'ATBPServer', 'static', 'assets', 'pfp');

      try {
        const files = fs.readdirSync(iconsPath);
        files.forEach(file => {
          const iconName = path.parse(file).name;
          if (path.extname(file).toLowerCase() === '.jpg' && !ICONS.includes(iconName)) {
            ICONS.push(iconName);
          }
        });
    ...
    app.post('/icons/buy', async (req, res) => {
      ...
      try {
        ...
        if (!userId || !iconName) {
          return res.status(400).json({ message: 'Missing userId or iconName in request body' });
        }
        ...
        if (!iconDataFromServer) {
          return res.status(404).json({ message: `Icon '${iconName}' is not a valid icon name.` });
        }

        if (!iconDataFromServer.cost === undefined) {
          return res.status(400).json({ message: `Icon '${iconName}' does not have a cost.` });
        }
        ...
        if (currentCoins < COMMON_ICON_COST) {
          return res.status(400).json({ message: `Not enough coins. Need ${COMMON_ICON_COST}, have ${currentCoins}` });
        }
        ...
        const updateResult = await playerCollection.updateOne(query, updateOperation);
        ...
        if (updateResult.modifiedCount === 1) {
          ...
          console.error(`Failed to update database for user ${userId} buying ${iconName}. Result:`, updateResult);
          return res.status(500).json({ message: 'Database update failed after checks' });
        }
    ...
    app.post('/icons/use', async (req, res) => {
      ...
      try {
        ...
        if (!ALL_ICONS_METADATA[iconName] && iconName !== 'Default') {
          return res.status(404).json({ message: `Icon '${iconName}' is not a valid icon.` });
        }
        ...
        if (!ownedIcons.includes(iconName) && iconName !== 'Default') {
          return res.status(403).json({ message: `You do not own the icon '${iconName}'` });
        }
        ...
        if (updateResult.modifiedCount === 1 || updateResult.matchedCount === 1) {
          res.status(200).json({ message: 'Icon set successfully!', newPfp: iconName });
        } else {
          console.error(`Failed to set pfp for user ${userId} to ${iconName}. Result:`, updateResult);
          return res.status(500).json({ message: 'Database update failed unexpectedly' });
        }

      } catch (error) {
        console.error('Error using icon:', error);
        res.status(500).json({ message: 'Server error during icon use', error: error.message });
      }
    });
      ...
      try {
          ...
          if (p.username === username) {
            ...
            try {
              ...
              if (data) {
                ...
                console.log(`Player ${username} found online but does not exist in DB during update`);
              }

            } catch (dbError) {
              console.error('Error fetching player data during presence update: ' + dbError);
            }
            break;
          }
        }
        ...
        if (!playerFound) {
          ...
          if (data != null) {
            ...
            console.log(`New player ${username} not found in DB. Not adding to online list`);
          }
        }
