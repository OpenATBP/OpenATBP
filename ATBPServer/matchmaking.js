...
module.exports = {
  ...
  searchForFullGame: function(players, teams, queueSize){
    ...
    var team = 0;
    var playerTeam = teams.find((t) => t.players.includes(prioritizedPlayer.player.teg_id));
    var validPlayers = [];
    if(playerTeam != undefined){
      for(var p of playerTeam.players){ //If player has a team, make sure they all get added to the game.
    ...
    for(var p of sortedPlayers){ //Search through all players searching for game type.
      if(validPlayers.find((vp) => vp.teg_id == p.player.teg_id) == undefined){ //Make sure that the player is not already added to the game.
        var searchTeam = teams.find((t) => t.players.includes(p.player.teg_id));
        if(searchTeam != undefined && team < 2){ //Runs if the player is part of a team and two teams are not already in the game.
          if(searchTeam.length + validPlayers.length <= queueSize){ // Checks to see if the entire team can be added to the game.
            for(var tp of searchTeam.players){ //Adds all team members to the queue.
              var teamUser = players.find((u) => u.player.teg_id == p);
              if(teamUser != undefined){
                var queueSearchObj = {
                  'teg_id': tp,
        ...
        }else if(searchTeam == undefined){ //Runs if the player does not have a team.
          var queueSearchObj = {
    ...
    if(validPlayers.length < queueSize && failedTeam != undefined){ //If there is not enough players, but there's a failed team, try adding the two and seeing if it works out.
      ...
      for(var p of validPlayers){
        if(p != validPlayers[0] && p.team != -1 && p.team == validPlayers[0].team) newPlayers.push(p);
      }
      ...
      for(var p of sortedPlayers){
        if(!p.player.onTeam && validPlayers.find(vp => vp.teg_id == p.player.teg_id) == undefined){
          var queueSearchObj = {
            'teg_id': p.player.teg_id,
            'elo': p.player.elo,
            'team': -1
  ...
  getTeams: function(players,teams,teamSize){
    ...
    function findTeams(team1, team2, index) {
        if (index === unassignedPlayers.length) {
            ...
            const avgElo1 = team1.reduce((sum, player) => sum + player.elo, 0) / teamSize;
            const avgElo2 = team2.reduce((sum, player) => sum + player.elo, 0) / teamSize;
