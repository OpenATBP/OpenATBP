...
public class Hunson extends UserActor {
    ...
    public void update(int msRan) {
        ...
        if(this.ultActivated){
            ...
            for(Actor a : Champion.getActorsInRadius(this.parentExt.getRoomHandler(this.room.getId()),this.location,4f)){
    ...
    private boolean hasStatusEffect(Actor a){
        ...
        for(ActorState s : states){
            if(a.getState(s) && s != ActorState.BRUSH && s != ActorState.TRANSFORMED && s != ActorState.REVEALED) return true;
        }
        return false;
    }
        ...
        switch(ability){
            case 1:
                ...
                if(!this.qActivated){
                    ...
                    this.qUses = 2;
                    SmartFoxServer.getInstance().getTaskScheduler().schedule(new HunsonAbilityHandler(ability,spellData,cooldown,gCooldown,dest),6000, TimeUnit.MILLISECONDS);
                }else{
                    this.qUses--;
                    if(this.qUses == 0){
            ...
            case 2:
                ...
                SmartFoxServer.getInstance().getTaskScheduler().schedule(new HunsonAbilityHandler(ability,spellData,cooldown,gCooldown,dest),castDelay,TimeUnit.MILLISECONDS);
                break;
            case 3:
    ...
    private class HunsonAbilityHandler extends AbilityRunnable {
        ...
        public HunsonAbilityHandler(int ability, JsonNode spellData, int cooldown, int gCooldown, Point2D dest) {
        ...
        protected void spellE() {
            canCast[2] = true;
            ExtensionCommands.createActorFX(parentExt,room,id,"hunson_soul_suck",3500,id+"_ultSuck",true,"",true,false,team);
    ...
    private class HudsonProjectile extends Projectile {
        ...
        public HudsonProjectile(ATBPExtension parentExt, UserActor owner, Line2D path, float speed, float hitboxRadius, String id) {
        ...
        public void destroy() {
            super.destroy();
            ExtensionCommands.createWorldFX(this.parentExt,room,id,"hunson_projectile_explode",id+"_destroyed",500,(float)this.location.getX(),(float)this.location.getY(),false,team,0f);
            canCast[0] = true;
        }
    }
}
