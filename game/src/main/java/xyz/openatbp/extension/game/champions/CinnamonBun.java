    ...
    public void useAbility(int ability, JsonNode spellData, int cooldown, int gCooldown, int castDelay, Point2D dest) {
        switch (ability){
            ...
            case 2:  //TODO: make target rect work
                ...
                this.changeHealth((int) ((double)(this.getMaxHealth())*0.05d));
                this.wLine = Champion.getAbilityLine(this.location,dest,6.5f);
                double slideX = Champion.getAbilityLine(this.location,dest,1.5f).getP2().getX();
                double slideY = Champion.getAbilityLine(this.location,dest,1.5f).getP2().getY();
                float rotation = getRotation(dest);
                Point2D dashEndPoint = this.dash(this.wLine.getP2(),false,15d);
                double dashTime = this.location.distance(dashEndPoint)/15d;
